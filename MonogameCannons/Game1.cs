...
namespace MonogameCannons
{
    ...
    public class Game1 : Game
    {
        ...
        private List<int> _smokeTime = new List<int>();
        ...
        private void GenerateTerrainContour()
        {
            ...
            {
                height = peakheight * Math.Sin((float)x / flatness * rand1 + rand1);
                height -= peakheight * Math.Cos((float)x / flatness * rand2 + rand2);
                height += peakheight * Math.Sin((float)x / flatness * rand3 + rand3);
                height += offset;
                _terrainContour[x] = (int)height;
            }
        }
        private void CreateForegound()
        {
            ...
            for (int x = 0; x < _screenWidth; ++x)
            {
                ...
                {
                    foregroundColors[x + y * _screenWidth] = (y > _terrainContour[x]) ?
                        groundColors[x % _groundTexture.Width, y % _groundTexture.Height] : Color.Transparent;
                }
            }
        ...
        private Matrix CreateRocketMat()
        {
        ...
        {
            Matrix rocketMat = CreateRocketMat();
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (_particleList.Count > 0) UpdateParticles(gameTime);
            if (!_rocketFlying && _particleList.Count == 0) ProcessKeyboard();
        ...
        private void UpdateRocket()
        {
            if (_rocketFlying)
            {
                if (_rocketPosition.X > _screenWidth + 20 || _rocketPosition.X < -20 || _rocketPosition.Y > _screenHeight + 20)
                {
                    _rocketFlying = false;
                }
        ...
        {
            int currentAngle = (int)MathHelper.ToDegrees(_players[_currentPlayer].Angle);
            int currentPower = (int)_players[_currentPlayer].Power;
            _spriteBatch.DrawString(_font, $"Cannon angle: {currentAngle}", new Vector2(20, 20), _players[_currentPlayer].Color);
            _spriteBatch.DrawString(_font, $"Cannon power: {currentPower}", new Vector2(20, 45), _players[_currentPlayer].Color);
        }
        ...
        {
            for (int i = _smokeList.Count - 1; i >= 0; --i)
            {
                if (--_smokeTime[i] <= 0)
                {
                    _smokeList.RemoveAt(0);
                    _smokeTime.RemoveAt(0);
                    continue;
                }
                _spriteBatch.Draw(_smokeTexture, _smokeList[i], null, Color.White * ((float)_smokeTime[i] / 75), 0, new Vector2(40, 35), 0.2f, SpriteEffects.None, 1);
            }
        }
